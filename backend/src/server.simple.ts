...
app.get('/health', (_req, res) => {
  res.json({
    success: true,
    message: 'Mission Control Backend (Simple) is running',
...
app.get('/api/auth/me', authenticate, async (req: any, res): Promise<void> => {
  try {
    const user = await prisma.user.findUnique({
...
// Missions: Get single
app.get('/api/missions/:id', async (req, res): Promise<void> => {
  try {
    const { id } = req.params;
...
app.post('/api/missions', authenticate, async (req: any, res): Promise<void> => {
  try {
...
app.put('/api/missions/:id', authenticate, async (req: any, res): Promise<void> => {
  try {
    const { id } = req.params;
...
app.delete('/api/missions/:id', authenticate, async (req: any, res): Promise<void> => {
  try {
    const { id } = req.params;
...
app.get('/api/players', authenticate, async (req: any, res): Promise<void> => {
  try {
    if (!['ADMIN', 'TRAINER'].includes(req.user.role)) {
      return res.status(403).json({
        success: false,
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Mission Control Backend (Simplified) running on port ${PORT}`);
